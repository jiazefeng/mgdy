...
<script>
    $(function () {
        $("#000400000000").addClass("active");
        $("#000400000000").parent().parent().addClass("in");
        $("#000400000000").parent().parent().parent().parent().addClass("active");
    })
    new WOW().init();
</script>
...
<div class="main-content">
    <vl:leftmenu sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>"
                 crunMenu="000400000000" username="${propertystaff.staffName}"/>
    <div class="forms">
    ...
    <div class="table-responsive bs-example widget-shadow">
        <table width="100%" class="tableStyle">
            ...
            <tbody>
            <c:forEach items="${houseInfoList}" var="house" varStatus="row">
                <tr>
                    <td><b>${(webPage.pageIndex-1)*20+row.index + 1}</b></td>
                    <td>${house.houseType}</td>
                    <td>${house.housePrice}</td>
                    <td>${house.groupBuyingPrice}</td>
                    <td>${house.houseDescribe}</td>
                    <td class="last">
                        <a href="javascript:void(0);" onclick="toEdit('${house.houseId}')" id="toEdit" class="a3">编辑</a>
                        <a href="javascript:void(0);" onclick="toDelete('${house.houseId}')" id="toDelete"
                           class="a3">删除</a>
                    </td>
                </tr>
            </c:forEach>
        ...
        <m:pager pageIndex="${webPage.pageIndex}"
                 ...
                 submitUrl="${pageContext.request.contextPath }/house/getHouseInfoList.html?pageIndex={0}"/>
    </div>
...
<script>
    //修改
    function toEdit(id) {
        window.location.href = "../house/toModifyHouse.html?houseId=" + id;
    }
    //删除
    function toDelete(id) {
        if (confirm("确定要删除吗?")) {
            window.location.href = "../house/deleteHouseInfo.html?houseId=" + id;
        }
    }
</script>
</body>
